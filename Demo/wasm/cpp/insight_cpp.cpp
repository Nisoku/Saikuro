...
static int write_json_ngrams(KV *table, int table_len, int top_n, char *out,
                             int cap) {
  int order[256];
  int n = 0;
...
extern "C" int compute_ngrams(const char *text, int top_n, char *out_buf,
                              int out_capacity) {
  char tokens[512][64];
  ...
  wc('{');
  ws("\"bigrams\":");
  text_pos = write_json_ngrams(bigrams, 256, top_n, tmp, sizeof(tmp) - 1);
  tmp[text_pos] = '\0';
  ws(tmp);
  wc(',');
  ws("\"trigrams\":");
  text_pos = write_json_ngrams(trigrams, 256, top_n, tmp, sizeof(tmp) - 1);
  tmp[text_pos] = '\0';
  ws(tmp);
  wc('}');
  if (pos < out_capacity)
...
saikuro_cpp_start_provider(const char *channel) {
  char address[256];
  {
  ...
  saikuro_provider_register_with_schema(provider, "ngrams", handle_ngrams, NULL,
                                        2, "Any");
  saikuro_provider_serve_async(provider, address, saikuro_cpp_serve_status,
                               NULL);
}
}
