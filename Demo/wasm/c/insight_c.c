  ...
  if (text) {
    while (text[bytes]) {
      ...
      if (c < 0x80) {
        ...
        non_ascii++;
        int len = 1;
        while (len < 3 && text[bytes + len] &&
               (text[bytes + len] & 0xC0) == 0x80)
          len++;
        bytes += len;
      } else if (c < 0xF8) {
        non_ascii++;
        int len = 1;
        while (len < 4 && text[bytes + len] &&
               (text[bytes + len] & 0xC0) == 0x80)
          len++;
        bytes += len;
      } else {
...
saikuro_c_start_provider(const char *channel) {
  char address[256];
  {
  ...
  saikuro_provider_register_with_schema(provider, "stats", handle_stats, NULL,
                                        1, "Any");
