...
use crate::wasi::tcp::{backend, WasiTcpConnector, WasiTcpListener, WasiTcpReceiver, WasiTcpSender};
...
pub struct WasiHostSend(pub WasiTcpSender<backend::Writer>);

/// Receiving half of a [`WasiPipe`] connection.
pub struct WasiHostRecv(pub WasiTcpReceiver<backend::Reader>);
        ...
        match role {
            ...
            Role::Accept => {
                ...
                let transport = listener
                    .accept()
                    .await?
                    .ok_or_else(|| TransportError::ConnectionLost("wasi-host listener closed".into()))?;
                let (mut tx, mut rx) = transport.split();
                match rx.recv().await? {
