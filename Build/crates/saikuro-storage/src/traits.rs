...
/// A key-value backend for single-threaded runtimes.
///
/// Unlike [`KeyValueBackend`], this trait uses native async functions and does
/// not require `Send` or `Sync`. Implementations must be used directly or
/// behind a statically selected generic type; they cannot be erased into a
/// host `Box<dyn StorageBackend>`.
#[allow(async_fn_in_trait)]
pub trait LocalKeyValueBackend: 'static {
    /// Get the configuration for this backend.
    fn config(&self) -> &StorageConfig;
...
/// A file backend for single-threaded runtimes.
#[allow(async_fn_in_trait)]
pub trait LocalFileBackend: 'static {
    /// Read a file's contents.
    async fn read_file(&self, path: &str) -> Result<Bytes>;
...
/// A single-threaded storage backend combining key-value and file operations.
#[allow(async_fn_in_trait)]
pub trait LocalStorageBackend: LocalKeyValueBackend {
    /// Check if this backend supports file operations.
    fn supports_files(&self) -> bool;
