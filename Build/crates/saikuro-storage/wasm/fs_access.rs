...
async fn pick_directory() -> Result<FileSystemDirectoryHandle> {
    ...
    let promise = js_sys::Reflect::get(&window, &JsValue::from_str("showDirectoryPicker"))
        .map_err(|e| SaikuroError::internal(format!("showDirectoryPicker not available: {e:?}")))?;

    let promise = promise
        .dyn_into::<js_sys::Function>()
        .map_err(|e| {
            SaikuroError::internal(format!("showDirectoryPicker is not a function: {e:?}"))
        })?
        .call0(&window)
        .map_err(|e| SaikuroError::internal(format!("showDirectoryPicker call failed: {e:?}")))?
    ...
    let result = promise_await(promise).await.map_err(|e| {
        SaikuroError::internal(format!("showDirectoryPicker promise failed: {e:?}"))
    })?;
...
async fn get_or_create_dir(
    ...
    let result = promise_await(promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("getOrCreateDir({name}) failed: {e:?}")))?;
    Ok(result.into())
}
...
async fn get_or_create_file(
    ...
    let result = promise_await(promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("getOrCreateFile({name}) failed: {e:?}")))?;
    Ok(result.into())
}
...
async fn read_file_from_handle(file: &FileSystemFileHandle) -> Result<Bytes> {
    ...
    let file_val = promise_await(file_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("getFile failed: {e:?}")))?;
    let js_file: web_sys::File = file_val.into();

    let buf_promise = js_file.array_buffer();
    let buf_val = promise_await(buf_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("arrayBuffer failed: {e:?}")))?;
...
async fn write_file_to_handle(file: &FileSystemFileHandle, data: &Bytes) -> Result<()> {
    ...
    let writable_val = promise_await(writable_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("createWritable failed: {e:?}")))?;
    let writable: web_sys::FileSystemWritableFileStream = writable_val.into();

    let write_promise = writable
        .write_with_u8_array(data)
        .map_err(|e| SaikuroError::internal(format!("write call failed: {e:?}")))?;
    promise_await(write_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("write failed: {e:?}")))?;

    promise_await(writable.close())
        .await
        .map_err(|e| SaikuroError::internal(format!("close failed: {e:?}")))?;
...
async fn append_file_to_handle(file: &FileSystemFileHandle, data: &Bytes) -> Result<()> {
    ...
    let file_val = promise_await(file_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("getFile(append) failed: {e:?}")))?;
    let js_file: web_sys::File = file_val.into();
    ...
    let writable_val = promise_await(writable_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("createWritable(append) failed: {e:?}")))?;
    let writable: web_sys::FileSystemWritableFileStream = writable_val.into();

    // Seek to end of file
    let seek_promise = writable
        .seek_with_f64(file_size)
        .map_err(|e| SaikuroError::internal(format!("seek(append) call failed: {e:?}")))?;
    promise_await(seek_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("seek(append) failed: {e:?}")))?;

    let write_promise = writable
        .write_with_u8_array(data)
        .map_err(|e| SaikuroError::internal(format!("write call(append) failed: {e:?}")))?;
    promise_await(write_promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("write(append) failed: {e:?}")))?;

    promise_await(writable.close())
        .await
        .map_err(|e| SaikuroError::internal(format!("close(append) failed: {e:?}")))?;
...
async fn remove_entry(parent: &FileSystemDirectoryHandle, name: &str) -> Result<()> {
    ...
    promise_await(promise)
        .await
        .map_err(|e| SaikuroError::internal(format!("removeEntry({name}) failed: {e:?}")))?;
    Ok(())
}
...
async fn remove_entry_recursive(parent: &FileSystemDirectoryHandle, name: &str) -> Result<()> {
    ...
    promise_await(promise).await.map_err(|e| {
        SaikuroError::internal(format!("removeEntry({name},recursive) failed: {e:?}"))
    })?;
    Ok(())
}
...
async fn list_entry_names(
    ...
    loop {
        let promise = iter
            .next()
            .map_err(|e| SaikuroError::internal(format!("iterator next() failed: {e:?}")))?;
        let result = promise_await(promise)
            .await
            .map_err(|e| SaikuroError::internal(format!("iterator promise failed: {e:?}")))?;
    ...
    async fn navigate_to_dir(
        ...
        for &dir_name in dirs {
            if create {
                ...
                let result = promise_await(promise)
                    .await
                    .map_err(|e| SaikuroError::internal(format!("navigateToDir failed: {e:?}")))?;
                current = result.into();
            } else {
                let opts = FileSystemGetDirectoryOptions::new();
                let promise = current.get_directory_handle_with_options(dir_name, &opts);
...
impl FileBackend for FsAccessStorage {
    ...
    async fn create_dir(&self, path: &str) -> Result<()> {
        ...
        promise_await(promise)
            .await
            .map_err(|e| SaikuroError::internal(format!("createDir({path}) failed: {e:?}")))?;
        Ok(())
    }
