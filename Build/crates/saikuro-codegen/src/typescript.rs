...
impl TypeScriptGenerator {
    fn type_to_ts_bare(&self, desc: &TypeDescriptor) -> String {
        convert_type(desc, &BareTypeConverter)
    }
    ...
    fn generate_types(&self, schema: &Schema) -> Result<String> {
        ...
        let type_to_ts = |desc: &TypeDescriptor| self.type_to_ts_bare(desc);
...
/// Converter that returns bare type names (no `Types.` prefix).
/// Used inside types.ts where types are defined, not imported.
struct BareTypeConverter;

impl TypeConverter for BareTypeConverter {
