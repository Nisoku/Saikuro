 ...
 * VERY important: The returned string must be allocated with saikuro_string_dup
 * so the adapter can reclaim it safely.
 */
typedef char *(SAIKURO_CALL *saikuro_provider_handler_fn)(
    void *user_data, const char *args_json);

/* Shared string helpers.
 * Ownership contract: functions returning char* transfer ownership to the
 * caller. Free returned pointers with saikuro_string_free (including
 * saikuro_string_dup, saikuro_last_error_message, and
 * saikuro_client_call_json-style return values).
 */
...
/* Client API */
SAIKURO_API saikuro_client_t SAIKURO_CALL
saikuro_client_connect(const char *address);
...
SAIKURO_API char *SAIKURO_CALL saikuro_client_call_json(saikuro_client_t handle,
                                                        const char *target,
                                                        const char *args_json);

SAIKURO_API char *SAIKURO_CALL
saikuro_client_call_json_timeout(saikuro_client_t handle, const char *target,
                                 const char *args_json, int timeout_ms);

SAIKURO_API int SAIKURO_CALL saikuro_client_cast_json(saikuro_client_t handle,
                                                      const char *target,
                                                      const char *args_json);

SAIKURO_API char *SAIKURO_CALL
saikuro_client_batch_json(saikuro_client_t handle, const char *calls_json);

SAIKURO_API saikuro_stream_t SAIKURO_CALL saikuro_client_stream_json(
    saikuro_client_t handle, const char *target, const char *args_json);

SAIKURO_API saikuro_channel_t SAIKURO_CALL saikuro_client_channel_json(
    saikuro_client_t handle, const char *target, const char *args_json);

SAIKURO_API int SAIKURO_CALL
saikuro_channel_send_json(saikuro_channel_t channel, const char *item_json);
...
SAIKURO_API int SAIKURO_CALL saikuro_channel_next_json(
    saikuro_channel_t channel, char **out_item_json, int *out_done);

SAIKURO_API void SAIKURO_CALL saikuro_channel_free(saikuro_channel_t channel);

/** Same contract as saikuro_channel_next_json. */
SAIKURO_API int SAIKURO_CALL saikuro_stream_next_json(saikuro_stream_t stream,
                                                      char **out_item_json,
                                                      int *out_done);
...
SAIKURO_API int SAIKURO_CALL saikuro_client_log(saikuro_client_t handle,
                                                const char *level,
                                                const char *name,
                                                const char *msg,
...
/* Provider API */
SAIKURO_API saikuro_provider_t SAIKURO_CALL
saikuro_provider_new(const char *namespace_name);
