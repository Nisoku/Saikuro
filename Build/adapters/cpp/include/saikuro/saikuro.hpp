...
template <typename Derived, typename Handle> class MoveOnlyHandle {
public:
...
public:
  ...
  private:
    ...
    void destroy_impl() {
      if (open_) {
        std::ignore = saikuro_channel_close(handle_);
      }
  ...
  std::string call_json(const std::string &target,
                        const std::string &args_json) const {
    char *result =
        saikuro_client_call_json(handle_, target.c_str(), args_json.c_str());
    if (result == nullptr) {
      throw Error(last_error());
    }
  ...
  std::string call_json_timeout(const std::string &target,
                                const std::string &args_json,
                                int timeout_ms) const {
    char *result = saikuro_client_call_json_timeout(
        handle_, target.c_str(), args_json.c_str(), timeout_ms);
    if (result == nullptr) {
      throw Error(last_error());
    }
    return take_owned_c_string(result);
  ...
  void cast_json(const std::string &target,
                 const std::string &args_json) const {
    if (saikuro_client_cast_json(handle_, target.c_str(), args_json.c_str()) !=
        0) {
      throw Error(last_error());
    }
  }
  ...
  Stream stream_json(const std::string &target,
                     const std::string &args_json) const {
    saikuro_stream_t stream =
        saikuro_client_stream_json(handle_, target.c_str(), args_json.c_str());
    if (stream == nullptr) {
      throw Error(last_error());
    }
  ...
  Channel channel_json(const std::string &target,
                       const std::string &args_json) const {
    saikuro_channel_t channel =
        saikuro_client_channel_json(handle_, target.c_str(), args_json.c_str());
    if (channel == nullptr) {
      throw Error(last_error());
    }
  ...
  std::string resource_json(const std::string &target,
                            const std::string &args_json) const {
    char *result = saikuro_client_resource_json(handle_, target.c_str(),
                                                args_json.c_str());
    if (result == nullptr) {
      throw Error(last_error());
    }
  ...
  void log(const std::string &level, const std::string &name,
           const std::string &msg,
           const std::string &fields_json = "{}") const {
    if (saikuro_client_log(handle_, level.c_str(), name.c_str(), msg.c_str(),
                           fields_json.c_str()) != 0) {
      throw Error(last_error());
    }
  }
...
public:
  ...
  void register_handler(const std::string &name,
    ...
    if (saikuro_provider_register(handle_, name.c_str(), callback, user_data) !=
        0) {
      throw Error(last_error());
    }
  }
