...
#[test]
fn send_and_receive_single_frame() {
...
#[test]
fn multiple_frames_arrive_in_order() {
    saikuro_exec::block_on(async {
        let (a, b) = MemoryTransport::connected_pair();
        let (mut sender, _) = a.split();
        let (_, mut receiver) = b.split();
...
#[test]
fn empty_frame_is_transmitted_faithfully() {
    saikuro_exec::block_on(async {
        let (a, b) = MemoryTransport::connected_pair();
        let (mut sender, _) = a.split();
        let (_, mut receiver) = b.split();
...
#[test]
fn large_frame_is_transmitted_faithfully() {
    saikuro_exec::block_on(async {
        let (a, b) = MemoryTransport::connected_pair();
        let (mut sender, _) = a.split();
        let (_, mut receiver) = b.split();
...
#[test]
fn close_sender_signals_eof_to_receiver() {
    saikuro_exec::block_on(async {
        let (a, b) = MemoryTransport::connected_pair();
        let (mut sender, _) = a.split();
        let (_, mut receiver) = b.split();
...
#[test]
fn named_pair_label_does_not_affect_behaviour() {
    saikuro_exec::block_on(async {
