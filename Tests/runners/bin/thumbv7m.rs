...
fn main() -> ! {
    ...
    saikuro_qemu_tests::host_println("saikuro QEMU tests (thumbv7m)");

    // Init random source first (shared tests create Envelopes which need UUIDs)
    let _ = saikuro_random::init_from(&saikuro_qemu_tests::FixedEntropy);

    // Shared sync tests (schema, router, storage util, runtime basics)
    let (shared_passed, shared_failed) = saikuro_qemu_tests::run_shared_sync_tests();

    // QEMU-specific sync tests (random)
