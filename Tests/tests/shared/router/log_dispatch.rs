//! Log-envelope dispatch tests.

use crate::check_test;
use crate::TestSuite;
use crate::Box;
use core::task::Poll;
use futures::{pin_mut, poll};
use saikuro_core::Arc;
use saikuro_core::{
    envelope::{Envelope, InvocationType},
    InvocationId, ResponseEnvelope, PROTOCOL_VERSION,
};
use saikuro_event::{LogLevel, LogRecord, LogSink, Value};
use saikuro_exec::mpsc;
use saikuro_router::{
    provider::{ProviderHandle, ProviderRegistry, ProviderWorkItem},
    router::{InvocationRouter, RouterConfig},
};
use spin::Mutex;

type Captured = Arc<Mutex<crate::Vec<LogRecord>>>;

pub fn register(suite: &mut TestSuite) {
    suite.register(
        "router::log_envelope_is_not_routed_to_provider",
        log_envelope_is_not_routed_to_provider,
    );
    suite.register(
        "router::log_envelope_delivers_record_to_sink",
        log_envelope_delivers_record_to_sink,
    );
    suite.register(
        "router::log_all_levels_are_forwarded",
        log_all_levels_are_forwarded,
    );
    suite.register(
        "router::log_envelope_with_no_args_returns_ok_without_panicking",
        log_envelope_with_no_args_returns_ok_without_panicking,
    );
    suite.register(
        "router::log_envelope_with_invalid_args_returns_ok_without_panicking",
        log_envelope_with_invalid_args_returns_ok_without_panicking,
    );
    suite.register(
        "router::router_with_custom_sink_still_routes_calls",
        router_with_custom_sink_still_routes_calls,
    );
    suite.register(
        "router::multiple_log_envelopes_all_delivered_to_sink",
        multiple_log_envelopes_all_delivered_to_sink,
    );
}

struct CapturingSink {
    captured: Captured,
}

#[cfg_attr(not(feature = "embedded"), async_trait::async_trait)]
#[cfg_attr(feature = "embedded", async_trait::async_trait(?Send))]
impl LogSink for CapturingSink {
    async fn emit(&self, record: &LogRecord) {
        self.captured.lock().push(record.clone());
    }
}

fn capturing_sink() -> (CapturingSink, Captured) {
    let captured: Captured = Arc::new(Mutex::new(crate::Vec::new()));
    let sink = CapturingSink {
        captured: captured.clone(),
    };
    (sink, captured)
}

/// Construct a `Log`-type envelope carrying a [`LogRecord`].
fn make_log_envelope(level: LogLevel, name: &str, msg: &str) -> Envelope {
    let record = LogRecord::new("2026-01-01T00:00:00.000Z", level, name, msg);
    // Serialize the record to a Value::Map so it can live in args.
    let bytes = saikuro_core::to_vec(&record).expect("serialize LogRecord");
    let value: Value = saikuro_core::from_slice(&bytes).expect("deserialize to Value");

    Envelope {
        version: PROTOCOL_VERSION,
        invocation_type: InvocationType::Log,
        id: InvocationId::new().expect("entropy available"),
        target: "$log".into(),
        args: crate::vec![value],
        meta: Default::default(),
        capability: None,
        batch_items: None,
        stream_control: None,
        seq: None,
    }
}

fn make_router_with_sink(sink: CapturingSink) -> InvocationRouter<CapturingSink> {
    let registry = ProviderRegistry::new();
    InvocationRouter::<CapturingSink>::with_log_sink(registry, RouterConfig::default(), sink)
}

fn log_envelope_is_not_routed_to_provider() -> Result<(), &'static str> {
    crate::block_on(async {
        // Even with a registered provider, a Log envelope must NOT reach it.
        let (work_tx, mut work_rx) = mpsc::channel::<ProviderWorkItem>(crate::common::capacity(8));
        let handle = ProviderHandle::new("logger", crate::vec!["$log".into()], work_tx);
        let registry = ProviderRegistry::new();
        registry.register(handle).await;

        let (sink, _captured) = capturing_sink();
        let router = InvocationRouter::<CapturingSink>::with_log_sink(
            registry,
            RouterConfig::default(),
            sink,
        );

        let env = make_log_envelope(LogLevel::Info, "test.logger", "hello from test");
        let resp = router.dispatch(env).await;

        check_test!(resp.ok, "log dispatch should return ok_empty");

        // Provider channel must be empty: the log was NOT forwarded to it.
        let recv_fut = work_rx.recv();
        pin_mut!(recv_fut);
        check_test!(
            matches!(poll!(recv_fut.as_mut()), Poll::Pending),
            "log envelope must not be forwarded to any provider"
        );
        Ok(())
    })
}

fn log_envelope_delivers_record_to_sink() -> Result<(), &'static str> {
    crate::block_on(async {
        let (sink, captured) = capturing_sink();
        let router = make_router_with_sink(sink);

        let env = make_log_envelope(LogLevel::Warn, "myapp.handler", "something fishy");
        let resp = router.dispatch(env).await;

        check_test!(resp.ok, "log dispatch should succeed");

        let records = captured.lock();
        assert_eq!(records.len(), 1, "exactly one record should be captured");
        let r = &records[0];
        assert_eq!(r.level, LogLevel::Warn);
        assert_eq!(r.name, "myapp.handler");
        assert_eq!(r.msg, "something fishy");
        Ok(())
    })
}

fn log_all_levels_are_forwarded() -> Result<(), &'static str> {
    crate::block_on(async {
        let (sink, captured) = capturing_sink();
        let router = make_router_with_sink(sink);

        let levels = [
            LogLevel::Trace,
            LogLevel::Debug,
            LogLevel::Info,
            LogLevel::Warn,
            LogLevel::Error,
        ];

        for &level in &levels {
            let env = make_log_envelope(level, "level.test", "msg");
            let resp = router.dispatch(env).await;
            check_test!(resp.ok, "log dispatch should succeed");
        }

        let records = captured.lock();
        assert_eq!(records.len(), levels.len(), "all levels should be captured");
        for (i, &expected_level) in levels.iter().enumerate() {
            assert_eq!(records[i].level, expected_level);
        }
        Ok(())
    })
}

fn log_envelope_with_no_args_returns_ok_without_panicking() -> Result<(), &'static str> {
    crate::block_on(async {
        let (sink, captured) = capturing_sink();
        let router = make_router_with_sink(sink);

        // Malformed: no args at all.
        let env = Envelope {
            version: PROTOCOL_VERSION,
            invocation_type: InvocationType::Log,
            id: InvocationId::new().expect("entropy available"),
            target: "$log".into(),
            args: crate::vec![],
            meta: Default::default(),
            capability: None,
            batch_items: None,
            stream_control: None,
            seq: None,
        };
        let resp = router.dispatch(env).await;

        // Must not panic; ok_empty is returned.
        check_test!(resp.ok, "malformed log should still return ok");

        // The router logs a warning about the malformed record.
        check_test!(
            !captured.lock().is_empty(),
            "malformed log should emit a warning to sink"
        );
        Ok(())
    })
}

fn log_envelope_with_invalid_args_returns_ok_without_panicking() -> Result<(), &'static str> {
    crate::block_on(async {
        let (sink, captured) = capturing_sink();
        let router = make_router_with_sink(sink);

        // args[0] is a plain string, not a LogRecord map.
        let env = Envelope {
            version: PROTOCOL_VERSION,
            invocation_type: InvocationType::Log,
            id: InvocationId::new().expect("entropy available"),
            target: "$log".into(),
            args: crate::vec![Value::String("not a log record".into())],
            meta: Default::default(),
            capability: None,
            batch_items: None,
            stream_control: None,
            seq: None,
        };
        let resp = router.dispatch(env).await;

        check_test!(resp.ok, "invalid log args should still return ok");
        check_test!(
            !captured.lock().is_empty(),
            "invalid log args should emit a warning to sink"
        );
        Ok(())
    })
}

fn router_with_custom_sink_still_routes_calls() -> Result<(), &'static str> {
    crate::block_on(async {
        // A custom log sink must not interfere with normal call routing.
        let (work_tx, work_rx) = mpsc::channel::<ProviderWorkItem>(crate::common::capacity(8));
        let handle = ProviderHandle::new("math", crate::vec!["math".into()], work_tx);
        let registry = ProviderRegistry::new();
        registry.register(handle).await;

        // Spawn an auto-responder.
        saikuro_exec::spawn(async move {
            let mut rx = work_rx;
            while let Some(item) = rx.recv().await {
                if let Some(tx) = item.response_tx {
                    let _ = tx.send(ResponseEnvelope::ok(item.envelope.id, Value::Int(99)));
                }
            }
        });

        let (sink, _captured) = capturing_sink();
        let router = InvocationRouter::<CapturingSink>::with_log_sink(
            registry,
            RouterConfig::default(),
            sink,
        );

        let env = Envelope::call("math.compute", crate::vec![]).map_err(|_| "create")?;
        let resp = router.dispatch(env).await;
        check_test!(resp.ok, "call should still succeed with custom log sink");
        assert_eq!(resp.result, Some(Value::Int(99)));
        Ok(())
    })
}

fn multiple_log_envelopes_all_delivered_to_sink() -> Result<(), &'static str> {
    crate::block_on(async {
        let (sink, captured) = capturing_sink();
        let router = make_router_with_sink(sink);

        for i in 0..10u32 {
            let env = make_log_envelope(LogLevel::Info, "bulk.test", &crate::format!("message {i}"));
            let resp = router.dispatch(env).await;
            check_test!(resp.ok, "log dispatch should succeed");
        }

        let records = captured.lock();
        assert_eq!(records.len(), 10, "all 10 log records should be captured");
        for (i, record) in records.iter().enumerate() {
            assert_eq!(record.msg, crate::format!("message {i}"));
        }
        Ok(())
    })
}